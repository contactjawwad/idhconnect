...
document.addEventListener('DOMContentLoaded', function () {  
    ...
    const gridOptions = {
        columnDefs: [
            ...
            { headerName: "Slot", field: "Slot", filter: true },
            { headerName: "CF No", field: "CF No", filter: true },
            { headerName: "Capacity (GB)", field: "Capacity (GB)", filter: true },
            { headerName: "CF Part Number", field: "CF Part Number", filter: true },
    ...
    const summaryGridOptions = {
        // Removed enableRangeSelection as it is part of enterprise features
        clipboardDelimiter: '\t',
        columnDefs: [
            { headerName: "CF Part Number", field: "CF Part Number", filter: true },
        ...
        onGridReady: function (params) {
            ...
            params.api.autoSizeColumns(['QTY']);
            params.api.setGridOption('loading', true);
        }
    };
    ...
    function loadNextChunk() {
        ...
        fetch(`/reports/flash/data?start=${currentStart}&length=${chunkSize}`)
            .then(response => {
                if (!response.ok) {
            ...
            .then(data => {
                ...
                if (data.summary_data) {
                    ...
                    summaryGridApi.applyTransaction({ add: data.summary_data });
    ...
    window.exportFullReport = async function () {
        ...
        const headers = ["Site Name", "Slot", "CF No", "Capacity (GB)","CF Part Number","Shelf Type"];
        ...
        mainGridData.forEach(data => {
            const row = worksheet.addRow([data["Site Name"], data["Slot"], data["CF No"], data["Capacity (GB)"],data["CF Part Number"],data["Shelf Type"]]);
        ...
        const summaryHeaders = ["CF Part Number", "QTY", "Description"];
        ...
        summaryGridData.forEach(data => {
            const row = summarySheet.addRow([data["CF Part Number"], data["QTY"], data["Description"]]);
            row.eachCell((cell, colNumber) => {
                cell.fill = {
                    type: 'pattern',
                    pattern: 'solid',
        ...
        anchor.href = url;
        anchor.download = 'Flash_Memory_Report.xlsx';
        anchor.click();
